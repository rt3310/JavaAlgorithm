...
public class Q2357 {
    ...
    public static int getMaxNumber(int start, int end, int idx, int left, int right) {
        ...
        int mid = (start + end) / 2;
        return Math.max(getMaxNumber(start, mid, idx * 2, left, right), getMaxNumber(mid + 1, end, idx * 2 + 1, left, right));
    }
    ...
    public static int getMinNumber(int start, int end, int idx, int left, int right) {
        ...
        int mid = (start + end) / 2;
        return Math.min(getMinNumber(start, mid, idx * 2, left, right), getMinNumber(mid + 1, end, idx * 2 + 1, left, right));
    }
}
