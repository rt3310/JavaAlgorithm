...
public class Q11505 {
    ...
    public static long intervalMul(int start, int end, int idx, int left, int right) {
        ...
        int mid = (start + end) / 2;
        return (intervalMul(start, mid, idx * 2, left, right) * intervalMul(mid + 1, end, idx * 2 + 1, left, right)) % 1000000007;
    }
    ...
    public static long update(int start, int end, int idx, int updateIdx, long src) {
        ...
        int mid = (start + end) / 2;
        return tree[idx] = (update(start, mid, idx * 2, updateIdx, src) * update(mid + 1, end, idx * 2 + 1, updateIdx, src)) % 1000000007;
    }
}
